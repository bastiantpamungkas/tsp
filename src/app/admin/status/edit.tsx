...
export default function Edit(props:any) {
    ...
    const handleRequestEdit = async () => {
        ...
        try {
            let response = await fetch('/api/status/' + props.data.id, {
              method: 'PUT',
              headers: {
          ...
          if (data.error) {
            ...
            props.hasNotification('Status has been updated','success')
            props.onModalClose()
          }
        } catch(err) {
          console.log(err)
    ...
    useEffect(() => {
        if (props.data.id) {
            setValues({...values, name: props.data.row.name, description: (props.data.row.description == null) ? '' : props.data.row.description})
        } else {
            ...
            </DialogTitle>
            {
              (loading) ? (
                ...
                <DialogContent>
                  ...
                  <TextField id="description" label="Description" fullWidth value={values.description} onChange={handleChange('description')} margin="normal" multiline rows={5}/>
                </DialogContent>
              )
            }
            {
