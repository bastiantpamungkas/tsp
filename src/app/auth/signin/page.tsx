...
export default function Index() {
  ...
  const clickSubmit = async () => {
    setLoading(true)
    const response = await signIn("credentials", { redirect: false, email: values.email, password: values.password, callbackUrl: ((searchParams.get('callbackUrl')) ? searchParams.get('callbackUrl') : '/') as string })
    if (response && response.error) {
      setNotification('Sign in failed. Check the details you provided are correct.','error')
      setLoading(false)
    } else {
  ...
  useEffect(() => {
    if (session) {
        router.push((responseUrl) ? responseUrl : '/')
    }
    setLoadingTheme(false)
  }, [router, session, responseUrl]);
